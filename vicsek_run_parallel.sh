...
Np=$(grep "Np = " vicsek_params.py | awk '{print $3}')
pmax=$(expr $Np - 1)
plist=$(seq 0 $pmax)

# Get save file name and remove double quotes
save_name=$(grep "save_name = " vicsek_params.py | awk '{print $3}')
save_name="${save_name%\"}"
save_name="${save_name#\"}"

# Run simulations
echo "-----> vicsek_main.py"; parallel python vicsek_main.py ::: $plist || exit 1
